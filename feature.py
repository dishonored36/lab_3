...
myFile = open('feature.txt', 'ab+')


loc_face = 'D:\machine learning\experiment\lab_3\datasets\gray/face/*.jpg'
coll_face = io.ImageCollection(loc_face)
loc_nonf = 'D:\machine learning\experiment\lab_3\datasets\gray/nonface/*.jpg'
coll_nonf = io.ImageCollection(loc_nonf)

for i in range(len(coll_face)):
    npd = NPDFeature(coll_face[i])
    fea_face = npd.extract()
    dump(fea_face, myFile)

for i in range(len(coll_nonf)):
    npd = NPDFeature(coll_nonf[i])
    fea_nonf = npd.extract()
    dump(fea_nonf, myFile)
